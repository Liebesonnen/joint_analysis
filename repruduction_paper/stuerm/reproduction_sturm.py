...
def rotate_points_xyz(points, angle_x, angle_y, angle_z, center):
   """依次绕 X, Y, Z 轴旋转（角度以弧度表示），旋转中心为 center"""
   pts = points - center
   Rx = np.array([
       [1, 0, 0],
...
def apply_screw_motion(points, angle, axis, origin, pitch):
   """螺旋运动：绕 axis 旋转 angle 同时沿 axis 平移 (pitch * angle/(2π))"""
...
###############################################################################
#                    各具体模型：Rigid, Prismatic, Revolute, GP               #
###############################################################################
...
class RevoluteModel(GenericLinkModel):
   def __init__(self):
       ...
       self.state_params = (self.rot_center[0].item(), self.rot_center[1].item(), self.rot_center[2].item(),
                            self.rot_axis_q[0].item(), self.rot_axis_q[1].item(), self.rot_axis_q[2].item(), self.rot_axis_q[3].item(),
                            self.rot_radius,
                            self.rot_orientation_q[0].item(), self.rot_orientation_q[1].item(), self.rot_orientation_q[2].item(), self.rot_orientation_q[3].item(),
                            float(self.rot_mode))
   ...
   def guess_from_minimal_samples(self, samples):
       ...
       if self.rot_mode == 1:
           ...
           plane_center_4 = torch.tensor([onplane_center[0].item(), onplane_center[1].item(), c1_[2].item(), 1.0], dtype=torch.float32)
       ...
       self.center.x, self.center.y, self.center.z = self.rot_center[0].item(), self.rot_center[1].item(), self.rot_center[2].item()
   ...
   def refine_nonlinear(self, data):
       ...
       def closure():
           ...
           ll = self.compute_log_likelihood(data, estimate_gamma=True)
           loss = -ll + 0 * torch.sum(x0)  # 强制依赖，使loss具有梯度
           loss.backward()
           return loss
...
best_axis_pos_err = 999.  # Error de posición para el eje
best_axis_ang_err = 999.  # Error angular para el eje


def compute_axis_errors(model, mode):
    """
    Calcula los errores de posición y angular entre el eje estimado y el ground truth.
    Devuelve (position_error, angular_error)
    """
    position_error = 999.
    angular_error = 999.
    ...
    # Revolute Door 3
        ...
        gt_axis = gt_axis / np.linalg.norm(gt_axis)  # Normalizar
    ...
    # Para Planar Mouse y Ball Joint, no calculamos errores de ejes
    # ya que sus parámetros son diferentes (normal para planar, centro para ball)
...
# def visualize_joint_axis(model):
#     global joint_axis_viz
#     if joint_axis_viz is not None:
#         if ps.has_curve_network(joint_axis_viz):
#             ps.remove_curve_network(joint_axis_viz)
#         joint_axis_viz = None
...
#         p0 = pos
#         p1 = pos + dir_ * L
#         nodes = np.array([p0, p1])
#         edges = np.array([[0, 1]])
#         joint_axis_viz = f"{model.name}_axis"
#         ps.register_curve_network(joint_axis_viz, nodes, edges)
#         ps.get_curve_network(joint_axis_viz).set_enabled(True)
#     elif model.name == "revolute":
#         q = model.rot_axis_q
#         R = quaternion_to_matrix(q[0], q[1], q[2], q[3])
#         axis_ = R.dot(np.array([0, 0, 1]))
...
#        running = True
#    if running:
#        if frame_count[current_mode] < 60:
...
#    for (nm, bic, pe, oe) in all_model_info:
...
#        psim.TextUnformatted(f"PosErr = {best_pos_err:.3f}, OriErr = {best_ori_err:.3f}")
#        psim.TextUnformatted("误差计算方式: ")
#        psim.TextUnformatted("  位置误差 = 欧氏距离 (pose 与 fit)")
#        psim.TextUnformatted("  角度误差 = 两个姿态旋转矩阵的相对旋转角")
#        psim.TextUnformatted("  这样做一般合理, 当然也可再细分位姿加权等.")

def callback():
    global current_mode, running
    global best_model_name, best_model_bic, best_pos_err, best_ori_err
    global fitted_points_viz, all_model_info
    global best_axis_pos_err, best_axis_ang_err  # Nuevas variables globales
    ...
    if len(frames) >= 2:
        ...
        if model is not None:
            ...
            # Calcular errores separados para los ejes
    ...
    for info in all_model_info:
        if len(info) >= 6:  # Asegurarse de tener todos los valores
    ...
    if best_model_bic < 1e9:
        ...
        psim.TextUnformatted("误差计算方式: ")
        psim.TextUnformatted("  一般位置误差 = 欧氏距离 (pose 与 fit)")
        psim.TextUnformatted("  一般角度误差 = 两个姿态旋转矩阵的相对旋转角")
        psim.TextUnformatted("  轴位置误差 = 估计轴与groundtruth轴之间的最近距离")
        psim.TextUnformatted("  轴角度误差 = 估计轴与groundtruth轴之间的角度差值 (弧度)")


def dearpygui_thread():
   dpg.create_context()
   with dpg.window(label="Articulation Demo", width=600, height=300):
       dpg.add_text("此示例演示论文中的关节模型拟合.\nPolyscope 窗口用于 3D 可视化.")
