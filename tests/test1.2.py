...
def callback():
   ...
   if psim.TreeNodeEx("Noise & SuperGaussian + SG Filter", flags=psim.ImGuiTreeNodeFlags_DefaultOpen):
       ...
       changed_noise, new_noise_sigma = psim.InputFloat("Noise Sigma（mm）", noise_sigma*1000, 0.01)
       if changed_noise:
           noise_sigma = max(0.0, new_noise_sigma/1000)
   ...
   if current_mode == "Prismatic Door":
       ...
       if changed_x or changed_y or changed_z:
           ...
           joint_points = original_joint_points.copy()
           joint_points = rotate_points_xyz(joint_points, rotation_angle_x, rotation_angle_y, rotation_angle_z,
                                            joint_center)
           if noise_sigma > 1e-6:
