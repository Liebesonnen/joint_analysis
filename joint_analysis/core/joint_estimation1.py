...
def compute_planar_info(all_points_history, v_history, omega_history, w_i, device='cuda'):
    ...
    if not plane_is_fixed:
        ...
        mask = disp_norms > 0.1 * disp_mean  # 阈值设为平均位移的10%

        # 如果没有足够的有效位移，使用所有位移
        if torch.sum(mask) < 10:
            mask = torch.ones_like(mask, dtype=torch.bool)

        # 将位移向量重塑为2D张量(用于PCA)并应用掩码
        disps_filtered = []
        w_filtered = []
        ...
        for t in range(T - 1):
            ...
            if len(valid_indices) > 0:
                disps_filtered.append(displacements[t, valid_indices])
                w_filtered.append(w_i[valid_indices])

        if not disps_filtered:  # 如果没有有效位移，使用原方法
            # 原始PCA方法作为回退
            ...
            # 将所有过滤后的位移堆叠
            disp_stack = torch.cat(disps_filtered, dim=0)  # (K, 3) where K is total valid points
            w_stack = torch.cat(w_filtered, dim=0)  # (K,)
            w_sum = w_stack.sum() + 1e-9

            # 计算加权协方差矩阵
            wdisp = disp_stack * w_stack.unsqueeze(-1)  # 加权位移
            cov_mat = torch.einsum('bi,bj->ij', wdisp, disp_stack) / w_sum
            cov_mat += 1e-9 * torch.eye(3, device=device)
        ...
        # 改进：确保选择正确的特征向量
        # 平面运动应该集中在两个主方向，所以第三个特征值应该很小
        # 我们可以直接取最小特征值对应的特征向量作为法向量
        normal_ = eigvecs[:, 0]  # 对应最小特征值的向量
        normal_ = normalize_vector_torch(normal_.unsqueeze(0))[0]

        # 获取平面上的两个主方向
        axis1_ = eigvecs[:, 2]  # 对应最大特征值的向量
        axis2_ = eigvecs[:, 1]  # 对应中间特征值的向量
...
def compute_prismatic_info(all_points_history, v_history, omega_history, w_i, device='cuda'):
    ...
    confidence_weight = torch.pow(w_i, 2.0)  # 平方增强高权重点
    weighted_dir = torch.sum(max_vecs * confidence_weight.unsqueeze(-1), dim=0)
